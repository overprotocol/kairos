...
func TestDeleteBloomBits(t *testing.T) {
	...
	for i := uint(0); i < 2; i++ {
		for s := uint64(0); s < 2; s++ {
			...
			WriteBloomBits(db, i, s, params.DolphinGenesisHash, []byte{0x01, 0x02})
		}
	}
	...
	check(0, 1, params.DolphinGenesisHash, false)

	// Bit1 shouldn't be affect.
	check(1, 0, params.MainnetGenesisHash, true)
	check(1, 0, params.DolphinGenesisHash, true)
	check(1, 1, params.MainnetGenesisHash, true)
	check(1, 1, params.DolphinGenesisHash, true)
}
