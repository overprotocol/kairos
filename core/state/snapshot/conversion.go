...
type (
	...
	trieGeneratorFn func(db ethdb.KeyValueWriter, scheme string, epoch uint32, owner common.Hash, in chan (trieKV), out chan (common.Hash))

	// leafCallbackFn is the callback invoked at the leaves of the trie,
	// returns the subtrie root with the specified subtrie identifier.
	leafCallbackFn func(db ethdb.KeyValueWriter, accountHash, codeHash common.Hash, stat *generateStats) (common.Hash, bool, error)
)
...
func GenerateTrie(snaptree *Tree, root common.Hash, src ethdb.Database, dst ethdb.KeyValueWriter) error {
	...
	got, err := generateTrieRoot(dst, scheme, acctIt, epoch, common.Hash{}, stackTrieGenerate, func(dst ethdb.KeyValueWriter, accountHash, codeHash common.Hash, stat *generateStats) (common.Hash, bool, error) {
		// Migrate the code first, commit the contract code into the tmp db.
		if codeHash != types.EmptyCodeHash {
			code := rawdb.ReadCode(src, codeHash)
			if len(code) == 0 {
				return common.Hash{}, false, errors.New("failed to read contract code")
			}
		...
		hash, err := generateTrieRoot(dst, scheme, storageIt, epoch, accountHash, stackTrieGenerate, nil, stat, false)
		if err != nil {
			return common.Hash{}, false, err
		}
...
func generateTrieRoot(db ethdb.KeyValueWriter, scheme string, it Iterator, epoch uint32, account common.Hash, generatorFn trieGeneratorFn, leafCallback leafCallbackFn, stats *generateStats, report bool) (common.Hash, error) {
	var (
	...
	go func() {
		defer wg.Done()
		generatorFn(db, scheme, epoch, account, in, out)
	}()
		...
		if account == (common.Hash{}) {
			...
			if leafCallback == nil {
				...
				go func(hash common.Hash) {
					subroot, skip, err := leafCallback(db, hash, common.BytesToHash(account.CodeHash), stats)
					if err != nil {
						results <- err
						return
					}
					if !skip && account.Root != subroot {
...
func stackTrieGenerate(db ethdb.KeyValueWriter, scheme string, epoch uint32, owner common.Hash, in chan trieKV, out chan common.Hash) {
	options := trie.NewStackTrieOptions()
	if db != nil {
		options = options.WithWriter(func(path []byte, hash common.Hash, blob []byte) {
			rawdb.WriteTrieNode(db, epoch, owner, path, hash, blob, scheme)
		})
	}
