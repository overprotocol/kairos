...
type diskLayer struct {
	...
	genMarker  *generatorMarker          // Marker for the state that's indexed during initial layer generation
	genPending chan struct{}             // Notification channel when generation is done (test synchronicity)
	genAbort   chan chan *generatorStats // Notification channel to abort generating the snapshot in this layer

	deleteEpochData func(uint32) // Function to delete epoch data from disk
...
func (dl *diskLayer) Epoch() uint32 {
	return dl.epoch
}
...
// copying everything.
func (dl *diskLayer) Update(epoch uint32, blockHash common.Hash, destructs map[common.Hash]struct{}, accounts map[common.Hash][]byte, storage map[common.Hash]map[common.Hash][]byte) *diffLayer {
	return newDiffLayer(dl, epoch, blockHash, destructs, accounts, storage)
}

func accountCacheKey(epoch uint32, hash common.Hash) []byte {
	return append(common.Uint32ToBytes(epoch), hash[:]...)
}
