...
func testAccessList(t *testing.T, vals []struct{ k, v string }) {
	...
	root, nodes, _ = trie.Commit(false)
	db.Update(root, parent, 0, 0, trienode.NewWithNodeSet(nodes), nil)
...
func TestTinyTree(t *testing.T) {
	...
	db.Update(root, types.EmptyRootHash, 0, 0, trienode.NewWithNodeSet(set), nil)
	...
	root, set, _ = trie.Commit(false)
	db.Update(root, parent, 0, 0, trienode.NewWithNodeSet(set), nil)
