...
func TestSimulatedBeaconSendWithdrawals(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		tx, err := types.SignTx(types.NewTransaction(uint64(i), common.Address{}, big.NewInt(1000), params.TxGas, big.NewInt(params.MinimumBaseFee), nil), signer, testKey)
		if err != nil {
...
func TestOnDemandSpam(t *testing.T) {
	...
	// generate a bunch of transactions
	for i := 0; i < 20000; i++ {
		tx, err := types.SignTx(types.NewTransaction(uint64(i), common.Address{byte(i), byte(1)}, big.NewInt(1000), params.TxGas, big.NewInt(params.MinimumBaseFee*2), nil), signer, testKey)
		if err != nil {
