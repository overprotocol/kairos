...
func (b *EthAPIBackend) StateAndHeaderByNumber(ctx context.Context, number rpc.BlockNumber, withoutCkpt bool) (*state.StateDB, *types.Header, error) {
	...
	if withoutCkpt {
		stateDb, err = b.eth.BlockChain().StateAtWithoutCheckpoint(header)
	} else {
...
func (b *EthAPIBackend) StateAndHeaderByNumberOrHash(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash, withoutCkpt bool) (*state.StateDB, *types.Header, error) {
	if blockNr, ok := blockNrOrHash.Number(); ok {
...
func (b *EthAPIBackend) CalcEpoch(number uint64) (uint32, error) {
