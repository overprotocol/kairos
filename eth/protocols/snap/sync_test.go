...
type (
	accountHandlerFunc func(t *testPeer, requestId uint64, epoch uint32, root common.Hash, origin common.Hash, limit common.Hash, cap uint64) error
	storageHandlerFunc func(t *testPeer, requestId uint64, epoch uint32, root common.Hash, accounts []common.Hash, origin, limit []byte, max uint64) error
	trieHandlerFunc    func(t *testPeer, requestId uint64, epoch uint32, root common.Hash, paths []TrieNodePathSet, cap uint64) error
...
func (t *testPeer) RequestAccountRange(id uint64, epoch uint32, root, origin, limit common.Hash, bytes uint64) error {
	t.logger.Trace("Fetching range of accounts", "reqid", id, "root", root, "origin", origin, "limit", limit, "bytes", common.StorageSize(bytes))
	t.nAccountRequests++
	go t.accountRequestHandler(t, id, epoch, root, origin, limit, bytes)
	return nil
}

func (t *testPeer) RequestTrieNodes(id uint64, epoch uint32, root common.Hash, paths []TrieNodePathSet, bytes uint64) error {
	t.logger.Trace("Fetching set of trie nodes", "reqid", id, "root", root, "pathsets", len(paths), "bytes", common.StorageSize(bytes))
	t.nTrienodeRequests++
	go t.trieRequestHandler(t, id, epoch, root, paths, bytes)
	return nil
}

func (t *testPeer) RequestStorageRanges(id uint64, epoch uint32, root common.Hash, accounts []common.Hash, origin, limit []byte, bytes uint64) error {
	t.nStorageRequests++
	...
	go t.storageRequestHandler(t, id, epoch, root, accounts, origin, limit, bytes)
	return nil
}
...
func defaultTrieRequestHandler(t *testPeer, requestId uint64, epoch uint32, root common.Hash, paths []TrieNodePathSet, cap uint64) error {
	// Pass the response
	var nodes [][]byte
...
func defaultAccountRequestHandler(t *testPeer, id uint64, epoch uint32, root common.Hash, origin common.Hash, limit common.Hash, cap uint64) error {
...
func defaultStorageRequestHandler(t *testPeer, requestId uint64, epoch uint32, root common.Hash, accounts []common.Hash, bOrigin, bLimit []byte, max uint64) error {
...
func emptyRequestAccountRangeFn(t *testPeer, requestId uint64, epoch uint32, root common.Hash, origin common.Hash, limit common.Hash, cap uint64) error {
	t.remote.OnAccounts(t, requestId, nil, nil, nil)
	return nil
}

func nonResponsiveRequestAccountRangeFn(t *testPeer, requestId uint64, epoch uint32, root common.Hash, origin common.Hash, limit common.Hash, cap uint64) error {
	return nil
}

func emptyTrieRequestHandler(t *testPeer, requestId uint64, epoch uint32, root common.Hash, paths []TrieNodePathSet, cap uint64) error {
	t.remote.OnTrieNodes(t, requestId, nil)
	return nil
}

func nonResponsiveTrieRequestHandler(t *testPeer, requestId uint64, epoch uint32, root common.Hash, paths []TrieNodePathSet, cap uint64) error {
	return nil
}

func emptyStorageRequestHandler(t *testPeer, requestId uint64, epoch uint32, root common.Hash, accounts []common.Hash, origin, limit []byte, max uint64) error {
	t.remote.OnStorage(t, requestId, nil, nil, nil)
	return nil
}

func nonResponsiveStorageRequestHandler(t *testPeer, requestId uint64, epoch uint32, root common.Hash, accounts []common.Hash, origin, limit []byte, max uint64) error {
	return nil
}

func proofHappyStorageRequestHandler(t *testPeer, requestId uint64, epoch uint32, root common.Hash, accounts []common.Hash, origin, limit []byte, max uint64) error {
...
func starvingStorageRequestHandler(t *testPeer, requestId uint64, epoch uint32, root common.Hash, accounts []common.Hash, origin, limit []byte, max uint64) error {
	return defaultStorageRequestHandler(t, requestId, epoch, root, accounts, origin, limit, 500)
}

func starvingAccountRequestHandler(t *testPeer, requestId uint64, epoch uint32, root common.Hash, origin common.Hash, limit common.Hash, cap uint64) error {
	return defaultAccountRequestHandler(t, requestId, epoch, root, origin, limit, 500)
}
...
func corruptAccountRequestHandler(t *testPeer, requestId uint64, epoch uint32, root common.Hash, origin common.Hash, limit common.Hash, cap uint64) error {
...
func corruptStorageRequestHandler(t *testPeer, requestId uint64, epoch uint32, root common.Hash, accounts []common.Hash, origin, limit []byte, max uint64) error {
...
func noProofStorageRequestHandler(t *testPeer, requestId uint64, epoch uint32, root common.Hash, accounts []common.Hash, origin, limit []byte, max uint64) error {
...
func testSyncBloatedProof(t *testing.T, scheme string) {
	...
	nodeScheme, sourceAccountTrie, elems := makeAccountTrieNoStorage(epoch, 100, scheme)
	source := newTestPeer("source", t, term)
	source.accountTrie = sourceAccountTrie.Copy()
	source.accountValues = elems

	source.accountRequestHandler = func(t *testPeer, requestId uint64, epoch uint32, root common.Hash, origin common.Hash, limit common.Hash, cap uint64) error {
		var (
			proofs [][]byte
			keys   []common.Hash
			vals   [][]byte
	...
	if err := syncer.Sync(epoch, sourceAccountTrie.Hash(), false, cancel); err == nil {
...
func testSync(t *testing.T, scheme string) {
	...
	verifyTrie(epoch, scheme, syncer.db, sourceAccountTrie.Hash(), t)
}
...
func testSyncTinyTriePanic(t *testing.T, scheme string) {
	...
	nodeScheme, sourceAccountTrie, elems := makeAccountTrieNoStorage(epoch, 1, scheme)
...
func testSyncWithStorage(t *testing.T, scheme string) {
	...
	sourceAccountTrie, elems, storageTries, storageElems := makeAccountTrieWithStorage(epoch, scheme, 3, 3000, true, false, false)
...
func testMultiSyncManyUseless(t *testing.T, scheme string) {
	...
	sourceAccountTrie, elems, storageTries, storageElems := makeAccountTrieWithStorage(epoch, scheme, 100, 3000, true, false, false)
...
func testSyncBoundaryAccountTrie(t *testing.T, scheme string) {
	...
	nodeScheme, sourceAccountTrie, elems := makeBoundaryAccountTrie(epoch, scheme, 3000)
...
func testSyncNoStorageAndOneCappedPeer(t *testing.T, scheme string) {
	...
	nodeScheme, sourceAccountTrie, elems := makeAccountTrieNoStorage(epoch, 3000, scheme)
...
func testSyncBoundaryStorageTrie(t *testing.T, scheme string) {
	...
	sourceAccountTrie, elems, storageTries, storageElems := makeAccountTrieWithStorage(epoch, scheme, 10, 1000, false, true, false)
...
func testSyncWithStorageAndOneCappedPeer(t *testing.T, scheme string) {
	...
	sourceAccountTrie, elems, storageTries, storageElems := makeAccountTrieWithStorage(epoch, scheme, 300, 1000, false, false, false)
...
func testSyncWithStorageMisbehavingProve(t *testing.T, scheme string) {
	...
	nodeScheme, sourceAccountTrie, elems, storageTries, storageElems := makeAccountTrieWithStorageWithUniqueStorage(epoch, scheme, 10, 30, false)
...
func testSyncWithUnevenStorage(t *testing.T, scheme string) {
	...
	accountTrie, accounts, storageTries, storageElems := makeAccountTrieWithStorage(epoch, scheme, 3, 256, false, false, true)
	...
	mkSource := func(name string) *testPeer {
		...
		source.setStorageTries(storageTries)
		source.storageValues = storageElems
		source.storageRequestHandler = func(t *testPeer, reqId uint64, epoch uint32, root common.Hash, accounts []common.Hash, origin, limit []byte, max uint64) error {
			return defaultStorageRequestHandler(t, reqId, epoch, root, accounts, origin, limit, 128) // retrieve storage in large mode
		}
		return source
	}
	syncer := setupSyncer(scheme, mkSource("source"))
	if err := syncer.Sync(epoch, accountTrie.Hash(), false, cancel); err != nil {
		t.Fatalf("sync failed: %v", err)
	}
	verifyTrie(epoch, scheme, syncer.db, accountTrie.Hash(), t)
}
...
var (
	...
	uihashes = []common.Hash{
		crypto.Keccak256Hash([]byte{10}),
		crypto.Keccak256Hash([]byte{11}),
		crypto.Keccak256Hash([]byte{12}),
		crypto.Keccak256Hash([]byte{13}),
		crypto.Keccak256Hash([]byte{14}),
		crypto.Keccak256Hash([]byte{15}),
		crypto.Keccak256Hash([]byte{16}),
		crypto.Keccak256Hash([]byte{17}),
	}
)

// getCodeHash returns a pseudo-random code hash
func getCodeHash(i uint32) []byte {
	h := codehashes[int(i)%len(codehashes)]
	return common.CopyBytes(h[:])
}

// getUiHash returns a pseudo-random ui code hash
func getUiHash(i uint32) []byte {
	h := uihashes[int(i)%len(uihashes)]
	return common.CopyBytes(h[:])
}
...
func makeAccountTrieNoStorage(epoch uint32, n int, scheme string) (string, *trie.Trie, []*kv) {
	var (
	...
	db.Update(root, types.EmptyRootHash, epoch, 0, trienode.NewWithNodeSet(nodes), nil)

	accTrie, _ = trie.New(trie.StateTrieID(root, epoch), db)
...
// boundary hash.
func makeBoundaryAccountTrie(epoch uint32, scheme string, n int) (string, *trie.Trie, []*kv) {
	var (
		entries    []*kv
	...
	for i := 0; i < len(boundaries); i++ {
		value, _ := rlp.EncodeToBytes(&types.StateAccount{
			...
			Balance:       big.NewInt(int64(i)),
			Root:          types.EmptyRootHash,
			CodeHash:      getCodeHash(uint32(i)),
			UiHash:        getUiHash(uint32(i)),
			StorageCount:  0,
		})
...
func makeAccountTrieWithStorageWithUniqueStorage(epoch uint32, scheme string, accounts, slots int, code bool) (string, *trie.Trie, []*kv, map[common.Hash]*trie.Trie, map[common.Hash][]*kv) {
	var (
		db             = trie.NewDatabase(rawdb.NewMemoryDatabase(), newDbConfig(scheme))
		accTrie        = trie.NewEmpty(db)
		entries        []*kv
	...
	for i := uint32(1); i <= uint32(accounts); i++ {
		key := key32(i)
		codehash := types.EmptyCodeHash.Bytes()
		if code {
			codehash = getCodeHash(i)
		}
		uiHash := types.EmptyCodeHash.Bytes()
		if code {
			uiHash = getUiHash(i)
		}
		// Create a storage trie
		stRoot, stNodes, stEntries := makeStorageTrieWithSeed(epoch, common.BytesToHash(key), uint32(slots), i, db)
		nodes.Merge(stNodes)
	...
	db.Update(root, types.EmptyRootHash, epoch, 0, nodes, nil)

	// Re-create tries with new root
	accTrie, _ = trie.New(trie.StateTrieID(root, epoch), db)
	for i := uint32(1); i <= uint32(accounts); i++ {
		key := key32(i)
		id := trie.StorageTrieID(root, epoch, common.BytesToHash(key), storageRoots[common.BytesToHash(key)])
...
func makeAccountTrieWithStorage(epoch uint32, scheme string, accounts, slots int, code, boundary bool, uneven bool) (*trie.Trie, []*kv, map[common.Hash]*trie.Trie, map[common.Hash][]*kv) {
	var (
		db             = trie.NewDatabase(rawdb.NewMemoryDatabase(), newDbConfig(scheme))
		accTrie        = trie.NewEmpty(db)
		entries        []*kv
	...
	for i := uint32(1); i <= uint32(accounts); i++ {
		...
		if boundary {
			stRoot, stNodes, stEntries = makeBoundaryStorageTrie(epoch, common.BytesToHash(key), slots, db)
		} else if uneven {
			stRoot, stNodes, stEntries = makeUnevenStorageTrie(epoch, common.BytesToHash(key), slots, db)
		} else {
			stRoot, stNodes, stEntries = makeStorageTrieWithSeed(epoch, common.BytesToHash(key), uint32(slots), 0, db)
		}
		nodes.Merge(stNodes)
	...
	// Re-create tries with new root
	accTrie, err := trie.New(trie.StateTrieID(root, epoch), db)
	if err != nil {
		panic(err)
	}
	for i := uint32(1); i <= uint32(accounts); i++ {
...
func makeStorageTrieWithSeed(epoch uint32, owner common.Hash, n, seed uint32, db *trie.Database) (common.Hash, *trienode.NodeSet, []*kv) {
	trie, _ := trie.New(trie.StorageTrieID(types.EmptyRootHash, epoch, owner, types.EmptyRootHash), db)
	var entries []*kv
...
// boundary hash.
func makeBoundaryStorageTrie(epoch uint32, owner common.Hash, n int, db *trie.Database) (common.Hash, *trienode.NodeSet, []*kv) {
	var (
		entries    []*kv
		boundaries []common.Hash
		trie, _    = trie.New(trie.StorageTrieID(types.EmptyRootHash, epoch, owner, types.EmptyRootHash), db)
...
func makeUnevenStorageTrie(epoch uint32, owner common.Hash, slots int, db *trie.Database) (common.Hash, *trienode.NodeSet, []*kv) {
	var (
		entries []*kv
		tr, _   = trie.New(trie.StorageTrieID(types.EmptyRootHash, epoch, owner, types.EmptyRootHash), db)
...
func verifyTrie(epoch uint32, scheme string, db ethdb.KeyValueStore, root common.Hash, t *testing.T) {
	t.Helper()
	triedb := trie.NewDatabase(rawdb.NewDatabase(db), newDbConfig(scheme))
	accTrie, err := trie.New(trie.StateTrieID(root, epoch), triedb)
	if err != nil {
		t.Fatal(err)
	}
	accounts, slots := 0, 0
	...
	for accIt.Next() {
		...
		if acc.Root != types.EmptyRootHash {
			id := trie.StorageTrieID(root, epoch, common.BytesToHash(accIt.Key), acc.Root)
