...
if one is set.  Otherwise it prints the genesis from the datadir.`,
	...
	importCommand = &cli.Command{
		...
		Flags: flags.Merge([]cli.Flag{
			...
			utils.DisableHistoryFlag,
			utils.EpochLimitFlag,
		}, utils.DatabaseFlags),
		Description: `
...
func parseDumpConfig(ctx *cli.Context, stack *node.Node) (*state.DumpConfig, ethdb.Database, *types.Header, error) {
...
func dump(ctx *cli.Context) error {
	...
	chainConfig, err := core.LoadChainConfig(db, nil)
	if err != nil {
		return err
	}
	epoch, index := chainConfig.CalcEpochWithIndex(header.Number.Uint64())
	state, err := state.New(header.Root, header.CheckpointRoot, epoch, index, state.NewDatabaseWithNodeDB(db, triedb), nil)
	if err != nil {
		return err
	}
